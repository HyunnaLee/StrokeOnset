function [ result_measure, result_roc, likelihood ] = Step5_TestGLM(test_feature, test_label, feature_mean, feature_sd, selectedIdx, minimumInfarctSize, GLMmodel, GLMthrehold )
...
prob_estimates = predict(GLMmodel,SelectedFeatureSet);
[A, B, C, D, E] = perfcurve(test_label,prob_estimates(:,1),labelPositive);
result_roc.X = A;
result_roc.Y = B;
result_roc.T = C;
result_roc.AUC = D;
...
labelTestPredict(prob_estimates(:,1)>GLMthrehold) = labelPositive;
labelTestPredict(prob_estimates(:,1)<=GLMthrehold) = labelNegative;
